...
# --------------------------
# Step 1: Google Translate (Batch version with throttling)
# --------------------------
def process_google_file(data, target_languages, google_cache_path, console, log_file, batch_size=20, delay=0.5):
    """
    Efficient Google translation step using translate_batch with throttling.
    - Processes translations in batches per language.
    - Automatically respects Google free tier rate limits (~5 req/s).
    - Falls back to single translation if batch fails.
    """
    ...
    total_keys = len(data)
    console(f"[Google] Starting batch translation for {total_keys} keys and {len(target_languages)} languages...")
    ...
    for key, value in data.items():
        if isinstance(value, dict) and "original" in value:
            ...
            original = next(iter(value.values()))
        else:
            original = value
        cache.setdefault(key, {"original": original})

    # Process each target language in batches
    for lang in target_languages:
        console(f"[Google] Processing language: {lang}")
        untranslated = []
        keys_to_translate = []

        for key, entry in cache.items():
            if lang not in entry or not entry[lang]:
                untranslated.append(entry["original"])
                keys_to_translate.append(key)

        if not untranslated:
            console(f"[Google] All entries for {lang} already cached, skipping.")
            continue

        console(f"[Google] {len(untranslated)} items to translate for {lang}...")
            ...
            while not success and retries < 5:
                try:
                    ...
                    if not isinstance(translations, list):
                        raise ValueError("translate_batch did not return a list.")
                    success = True
                except Exception as e:
                    retries += 1
                    wait_time = min(5 * retries, 20)
                    console(f"[Google WARN] Batch failed (lang={lang}, size={current_batch_size}) attempt {retries}/5: {e}")
                    if "too many requests" in str(e).lower():
                        wait_time = 10 + retries * 5
                    sleep(wait_time)
                    if retries == 3 and current_batch_size > 5:
                        # shrink batch size dynamically if consistent failure
                        current_batch_size = max(5, current_batch_size // 2)
                        batch = batch[:current_batch_size]
                        batch_keys = batch_keys[:current_batch_size]
                        console(f"[Google] Reducing batch size to {current_batch_size} for {lang}")

            if not success:
                console(f"[Google ERROR] All batch retries failed for {lang}, switching to single-item fallback.")
                translations = []
                for text in batch:
                    try:
                        sleep(1)
                        translations.append(GoogleTranslator(source='auto', target=lang).translate(text))
                    except Exception as e2:
                        translations.append(f"[Google ERROR: {e2}]")

            # Save results incrementally
            for k, trans in zip(batch_keys, translations):
                cache[k][lang] = trans
                append_log(log_file, {"ts": now(), "step": "google", "key": k, "lang": lang, "translation": trans})
                console(f"[Google] {k} ({lang}): {trans[:80]}")
