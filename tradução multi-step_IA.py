...
# --------------------------
def process_google_file(data, target_languages, google_cache_path, console, log_file):
    """
    data: dict from input JSON (key -> original or nested)
    google_cache_path: path to cache/google.json
    """
    ...
    for key, value in data.items():
        ...
        # ensure original stored in cache structure
        if key not in cache:
            cache[key] = {"original": original}
